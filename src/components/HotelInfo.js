...
const HotelInfo = () => {
  ...
  const loadAccessibilityData = async () => {
    const response = await fetch('https://ryrfyi4hzb.execute-api.us-east-1.amazonaws.com/Production/accessibilities');
  ...
  const loadServicesData = async () => {
    const response = await fetch('https://ryrfyi4hzb.execute-api.us-east-1.amazonaws.com/Production/services');
    let jsonData = await response.json();
  ...
  return (
    <div className="scene" id="hotelinfo">
      ...
      <article id="greenprogram">
        ...
        <p>
          <strong>The Landon Hotel - London</strong> was recently renovated, and we considered the impact on the earth the entire way.
          From green building materials, to solar power, to energy-friendly lighting and appliances throughout the hotel - we’re saving
          energy in every socket, outlet, and switch. We’ve also initiated a recycling and composting program that reduces the load to
          local landfills, while providing valuable raw material for use in new products, or in the case of compost, for use in local
